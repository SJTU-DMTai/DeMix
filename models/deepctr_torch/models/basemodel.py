...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, shuffle=True, callbacks=None, sample_weights=None):
        """
        ...
        :param sample_weights: Numpy array of weights for the training samples.
        ...
        if sample_weights is not None:
            if validation_split and 0. < validation_split < 1.:
                sample_weights, _ = slice_arrays(sample_weights, 0, split_at), slice_arrays(sample_weights, split_at)
        else:
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            train_result = {}
            try:
                with tqdm(enumerate(train_loader), total=steps_per_epoch, disable=verbose != 1,
                          desc=f'Epoch {epoch+1}/{epochs}', ncols=120) as t:
                    for batch_idx, (x_train, y_train, w_train) in t:
                        x = x_train.to(self.device).float()
                        y = y_train.to(self.device).float()
                        w = w_train.to(self.device).float()
                        ...
                        if isinstance(loss_func, list):
                            ...
                            loss = sum(
                                [(loss_func[i](y_pred[:, i], y[:, i], reduction='none') * w).sum() for i in range(self.num_tasks)])
                        else:
                            loss = (loss_func(y_pred, y.squeeze(), reduction='none') * w).sum()
                        ...
                        # 实时更新进度条信息
                        postfix = {'loss': f'{total_loss_epoch / (batch_idx + 1) / batch_size:.4f}'}
                        if verbose > 0:
                            for name, metric_fun in self.metrics.items():
                                if name not in train_result:
                                    train_result[name] = []
                                metric_val = metric_fun(y.cpu().data.numpy(), y_pred.cpu().data.numpy().astype("float64"))
                                train_result[name].append(metric_val)
                                postfix[name] = f'{np.mean(train_result[name]):.4f}'
